    ...
    def trigger(self, conv_window, show_window):
        ...
        while conv_stride <= conv_win_size and show_win_size >= 0 and  \
              ...
              conv_window.get_item_by_raw_index(cid)[1].event_time +   \
                    self._max_conversion_delay:
            self._watermark = conv_window.\
                    get_item_by_raw_index(cid)[1].event_time
            conv_stride += step
...
class AttributionJoiner(ExampleJoiner):
    ...
    def _latest_attri(self, index):
        ...
        while lf < rt:
            mid = (lf + rt) // 2
            if index < self._sorted_attri_buf_by_leader_index[mid][1]:
                rt = mid
            else:
                lf = mid + 1
        return lf
    ...
    def _sort_and_evict_attri_buf(self, raw_matches, watermark):
        ...
        for (cid, sid) in raw_matches:
            ...
            if fe.event_time < watermark:
                #Drop the events of out-of-watermark
                continue

            self._dedup_by_follower_index[fi] = li
            # keep the latest show event
        ...
        for (fe, li, fi) in self._sorted_attri_buf_by_leader_index:
            if fe.event_time < watermark:
                if fi not in self._dedup_by_follower_index:
                    idx += 1
                    logging.warning("Unreachable here, li=%d, fi=%d", li, fi)
                    continue
